...
class PivotPoint2(bt.Indicator):
    ...
    def detect_peak(self, cur_date, cur_date_timestamp, offset_val, prev_predicted_time, prev_detected_type, min_datetime, max_datetime):
        ...
        if int(cur_date_timestamp) < bt.date2num(min_datetime) or int(cur_date_timestamp) > bt.date2num(max_datetime):
            #if prev_detected_type != "low" or prev_detected_type != "high":
            return "-1", "-1"
        else:
        ...
        for x1 in range(offset_val, 0):
            ...
            if self.data0.datetime.date(abs(x1)) == max_datetime or self.data0.datetime.date(x1) == min_datetime:
                ...
                if data.high[0] < data.high[x1] or data.high[0] < data.high[abs(x1)]: #add and if prev_detected_type == low
                    detect_highpeak_true = 0
                if data.low[0] > data.low[x1] or data.low[0] > data.low[abs(x1)]: #add and if prev_detected_type == high
                    detect_lowpeak_true = 0
            #if debug == 1:
        ...
        #if detect_lowpeak_true and prev_detected_type == "high":       #couse the prev_detected_type must be reverse from the current detection
        #    if bt.date2num(prev_detected_time) < cur_date_timestamp:   #so use prev_detected_time
        #elif detect_highpeak_true and prev_detected_type == "low":

        #highest peak detected
        #if detect_highpeak_true == 1 and detect_lowpeak_true == 0 and prev_detected_type == "high":
        #if detect_highpeak_true == 1 and detect_lowpeak_true == 0 and prev_detected_type == "-2" or prev_detected_type == "high":
        #if detect_highpeak_true == 1 and detect_lowpeak_true == 0 or prev_detected_type == "-2" and prev_detected_type == "high":
        if detect_highpeak_true == 1 and detect_lowpeak_true == 0 or prev_detected_type == "-2":
            #if prev_detected_type == "high":
                #start setting x1, y1
                ...
                if advance_detect_type == "high":
                    ...
                    #this high date will need to bring for next function use so
        ...
        #elif detect_highpeak_true == 0 and detect_lowpeak_true == 1 and prev_detected_type == "low": #detected low
        #elif detect_highpeak_true == 0 and detect_lowpeak_true == 1 and prev_detected_type == "-2" or prev_detected_type == "low":
        #elif detect_highpeak_true == 0 and detect_lowpeak_true == 1: #detected low
        #elif detect_highpeak_true == 0 and detect_lowpeak_true == 1 or prev_detected_type == "-2" and prev_detected_type == "low":
        elif detect_highpeak_true == 0 and detect_lowpeak_true == 1 or prev_detected_type == "-2":
            if prev_detected_type == "low":
                advance2low = 1
                advance_detect_type = 0
