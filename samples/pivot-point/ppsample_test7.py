...
class PivotPoint2(bt.Indicator):
    ...
    def next(self):
        ...
        if len(self.data0) > abs(offset_value):
            ...
            if k == "lowest": # this detection is the past offset value
                ...
                #x1_time = self.get_x_time(self.p.y1, "close")
                #x1_time_stamp = bt.date2num(x1_time)
                ...
                #x1_time = self.get_x_time(self.p.y1, "open")
                #x1_time_stamp = bt.date2num(x1_time)
                x1_time_stamp = self.PREV_X
                x2_time_stamp = date_timestamp_offset
                self.PREV_Y = self.p.y1
                ...
                if self.PREV_Y_state == "lowest":
                    #x1_time = self.get_x_time(self.p.y1, "open")
                    #x1_time_stamp = bt.date2num(x1_time)
                    #x1_time = self.data0.datetime.date(offset_value)
                    #x1_time_stamp = bt.date2num(x1_time)
                    ...
                    #x1_time = self.get_x_time(self.p.y1, "close")
                    #x1_time_stamp = bt.date2num(x1_time)
                    #x1_time = self.data0.datetime.date(-6)
                    #x1_time_stamp = bt.date2num(x1_time)
        ...
        self.lines.trend[0] = self.p.y1
        self.lines.signal[0] = self.p.y2
    ...
    #must write a function that predict the highest point in certain region
