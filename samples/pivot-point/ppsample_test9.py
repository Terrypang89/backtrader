...
class St(bt.Strategy):
    ...
    lines = ('trend', 'test')
    plotlines = dict(test=dict(_name='%D', ls='--'))
    plotinfo = dict(plot=True, plotlinevalues=True, subplot=False)
    #plotinfo = dict(plot=True, subplot=False)
    def __init__(self):
        #self.lines.plotinfo.plotmaster = self.data0
        #pprint(vars(self))
        ...
        self.p.plotinfo = dict(
                            ...
                            plotyticks = [],
                            subplot=False)
        self.p.plotlines = dict(trend = {}, test = {})
        self.p.lines = dict(trend = {}, test = {})
    ...
    def next(self):
        ...
        if len(self.data0) >= abs(offset_value):
            ...
            self.m = self.get_slope(x1_time_stamp, x2_time_stamp, self.p.y1, self.p.y2)
            self.B = self.get_y_intercept(self.m, x1_time_stamp, self.p.y1)
            self.p.plotlines.test[offset_value] = self.get_y(date_timestamp_offset)
        else:
            self.p.plotlines.test[0] = self.get_y(date_timestamp)
        self.p.plotlines.test[0] = self.p.y2
        #self.lines.signal[offset_value] = self.p.y2
    ...
    def detect_peak(self, cur_date, offset_val):
        ret_val = 0
        status = 0
        if offset_val is None or not offset_val or offset_val == "":
            offset_val = -6

        if self.data0.high[offset_val-1] <= self.data0.high[offset_val]:
            if self.data0.high[offset_val-2] <= self.data0.high[offset_val]:
                status = "highest"
            else:
                if self.data0.low[offset_val-1] > self.data0.low[offset_val] and self.data0.low[offset_val-2] > self.data0.low[offset_val]:
                    if self.data0.high[offset_val-3] <= self.data0.high[offset_val]:
                        status = "highest"
                    else:
                        status = "lowest"
                else:
        ...
        if self.data0.low[offset_val-1] >= self.data0.low[offset_val]:
            if self.data0.low[offset_val-2] >= self.data0.low[offset_val]:
                status = "lowest"
            else:
                if self.data0.high[offset_val-1] < self.data0.high[offset_val] and self.data0.high[offset_val-2] < self.data0.high[offset_val]:
                    if self.data0.low[offset_val-3] >= self.data0.low[offset_val]:
                        status = "lowest"
                    else:
                        status = "highest"
                else:
        ...
        for x2 in range(offset_val, -1):
            if len(self.data0) <= abs(offset_val)*4:
                ret_val = ret_val + 1
            else:
                if status == "highest":
                    if self.data0.high[offset_val] >= self.data0.high[x2+offset_val]:
                        ret_val = ret_val + (abs(offset_val) + x2 )
                    if self.data0.high[offset_val] >= self.data0.high[abs(x2)+ offset_val]:
                        ret_val = ret_val + abs(x2)
                elif status == "lowest":
                    if self.data0.low[offset_val] <= self.data0.low[x2+offset_val]:
                        ret_val = ret_val + (abs(offset_val) + x2 )
                    if self.data0.low[offset_val] <= self.data0.low[abs(x2)+ offset_val]:
                        ret_val = ret_val + abs(x2)
                print("detect_peak status=%s: ret_val=%s at date=%s, ori_date=%s" %(status, ret_val, self.data0.datetime.date(offset_val), cur_date))

        if ret_val >= abs(offset_val)*3:
            #print("detect_highest: ret_val=%s" %ret_val)
            return status # return true to current -6 value from
        else:
            return None
