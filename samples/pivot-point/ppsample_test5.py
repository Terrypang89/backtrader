...
class PivotPoint2(bt.Indicator):
    ...
    def __init__(self):
        ...
        bb = dataframe.loc[self.p.y1, "Date"]
    ...
    def next(self):
        ...
        if k == True or j == True:
        #if k == True:
            #if detect the true then start to get the new M value,
            # to get new M value will need new x2,x1,y2,y1.
            # y2, y1 can get is the previous HIghest/ Lowest value to latest updated value
            # then from those value y2, y1 to timeframe x2, x1,
            # then get new self.m and self.b

            self.p.y1 = self.PREV_Y
            self.p.y2 = self.data0.low[0]
            x1_time_stamp = get_x_time(self.p.y1)
    ...
    def get_x_time(self, y):
        # use self.datas[period], period to get back datetime
...
if __name__ == '__main__':
    ...
    dataframe.test = pd.DataFrame(dataframe, index = ['Date','Open','High','Low','Close','Volume','OpenInterest'])
