...
class PivotPoint2(bt.Indicator):
    ...
    def next(self):
        date_ori = self.datas[0].datetime.datetime()
        print("date_ori = %s, date_c = %s, datas length = %s, data0 length = %s" %(date_ori, self.data0.datetime.date(0), len(self.datas[0]), len(self.data0)))
        #if len(self.data0) > 6:
            #print(" datetimestamp[-6] = %s & date[-6]=%s" %(self.data0.datetime[-6], self.data0.datetime.date(-6)))
        ...
        if k == "lowest":
            ...
            x2_time_stamp = date_timestamp # must set
            self.m = self.get_slope(x1_time_stamp, x2_time_stamp, self.p.y1, self.p.y2)
            self.B = self.get_y_intercept(self.m, x1_time_stamp, self.p.y1)
            ...
            x1_time_stamp = get_x_timestamp(self.p.y1, "open")
            x2_time_stamp = date_timestamp #must get the timestamp[-6]
            self.m = self.get_slope(x1_time_stamp, x2_time_stamp, self.p.y1, self.p.y2)
            self.B = self.get_y_intercept(self.m, x1_time_stamp, self.p.y1)
            self.PREV_Y = self.p.y1
