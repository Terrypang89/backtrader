...
class PivotPoint2(bt.Indicator):
    ...
    def detect_prev_lowest(self, cur_date_timestamp, offset_val, lowest_time):
        ...
        if data.high[0] > data.high[-1] and data.high[0] > data.high[1]:
            ...
            largest_body_future_detected = 0
            largest_body_future = 0
            #look for future highest
            while True:
                if data.high[advance2high] > data.high[advance2high+1] and largest_body_future_detected == 1_and data.high[advance2high+1] > data.high[advance2high+2] and data.high[advance2high+2] > data.high[advance2high+3] and data.open[advance2high] > data.open[advance2high-1]
                #if data.open[advance2high] > data.open[advance2high+1] and data.open[advance2high] > data.open[advance2high-1]:
                    largest_advancehighbody = advance2high
                    self.largest_advancehighbody_date = date.high[advance2high].datetime
                    #compare with prev_largest_body through date, it is same?
                    advance_high_area = get_area(cur_dafe_timestamp, data.high[advance2high].datetime, data.close[advance2high], data.open[0])
                    break
                else:
                    if get_body_size(advance2high)/largest_body_future > 2:
                        largest_body_future_detected = 1
